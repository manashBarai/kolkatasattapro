...
const Fact = () => {
  const { fact, updatedAdArray } = useGlobalSkills()
  const initialState = {
    title: "",
    about: "",
    fees: "",
    ...
    try {
      if (edit === "") {
        ...
        const res = await axios.post(
          `${process.env.REACT_APP_API}fact`,
          formData
        );
        if (res.status === 200) {
          ...
          setPost(false);
          setEdit("")
          const updateArrays = [res.data, ...fact]
          updatedAdArray(updateArrays,'FACT')
        }
      } else {

        const res = await axios.put(
          `${process.env.REACT_APP_API}fact/${edit}`,
          formData
        );
        if (res.status === 200) {
          ...
          setFormData(initialState);
          setPost(false);
          const afterDelete = fact.filter(e => e._id !== edit)
                ...
                <button className="btn btn-sm btn-danger w-50" onClick={async () => {
                  try {
                    const res = await axios.delete(
                      `${process.env.REACT_APP_API}fact/${e._id}`,
                    );
                    if (res.status === 200) {
                      const afterDelete = fact.filter(advertise => advertise._id !== e._id)
                      updatedAdArray([...afterDelete],'FACT')
